...
def plot_market_price(df_chn):
    ...
    fig.update_layout(
        title=f"Carbon Price in China",
        template="none",
        showlegend=True,
        legend=dict(x=0.01, y=0.99),
        height=400,
        margin=dict(l=40, r=10, t=60, b=55),
        yaxis=dict(title="Price", tickprefix="￥", fixedrange=False),
        xaxis=dict(title="Date", fixedrange=False),
    )
    return fig
...
card_Cprice = dbc.Card(
    [
        # dbc.CardHeader("An Introduction to Asset Allocation"),
        dcc.Dropdown(id="market_select", multi=True, options=city_list, value=city_list, placeholder="Select market...", ),
        dcc.Graph(id='graph_Cprice', className="pb-4"),
    ],
    className="mt-4",#, width=12, lg=6,
)
@callback(
    Output("graph_Cprice", "figure"),
    Input("market_select", "value"),
)
def update_graph_Cprice(selected_markets):
    fig = go.Figure()
...
card_Xvars = dbc.Card(
    [
        dbc.CardHeader("A card for Xvars"),
        dbc.CardBody("This card will hold the Xvars"),
    ],
    className="mt-4",
)
...
card_forecast = dbc.Card(
    [
        dbc.CardHeader("A card for forecast"),
        dbc.CardBody("This card will hold the forecasts"),
    ],
    className="mt-4",
)
...
tabs = dbc.Tabs(
    [
        dbc.Tab([card_Cprice, card_Xvars], tab_id='tab-explore', label='Explore', id="tab-explore",),
        dbc.Tab([card_forecast], tab_id='tab-forecast', label='Forecast', id="tab-forecast",),
    ],
    id="my-tabs",
...
layout = dbc.Container(
    [
        ...
        dbc.Row(
            [
                dbc.Col(ref_tabs, width=12, lg=5, className="mt-4 border"),
                dbc.Col(
                    [
                        # dcc.Graph(id="allocation_pie_chart", className="mb-2"),
                        dcc.Graph(id="carbon_price_chart", className="pb-4"),
                        
                        dcc.Dropdown(id="xvars_select", multi=True, options=Xvar_list, value=["Brent-Oil", 'Rtd-Coal', 'USD-CNY'], placeholder="Select Xvars", ),
                        dcc.Graph(id="xvars_chart", className="pb-4"),
...
@callback(
    Output("xvars_chart", "figure"),
    Input("xvars_select", "value"),
)
def update_xvars_chart(xvars):
    fig = go.Figure()
    for x in xvars:
        fig.add_trace(
            go.Scatter(
...
@callback(
    ...
    Output("carbon_price_chart", "figure"),
    # Output("xvars_chart", "figure"),
...
def update_totals(stocks, cash, start_bal, planning_time, start_yr):
    ...
    fig_price = plot_market_price(df_chn)
    # fig_xvars = plot_xvars(df_chn)
    ...
    return data, fig_price, summary_table, ending_amount, ending_cagr
